...
def cv_ridge(Rstim, Rresp, Pstim, Presp, alphas, nfolds, chunklen, nchunks,
    ...
    """Uses ridge regression with a held-out set to get optimal alpha values for each response.
    [nchunks] random chunks of length [chunklen] will be taken from [Rstim] and [Rresp] for each regression
    run.  [nfolds] total regression runs will be performed.  The best alpha value for each response will be
    averaged across the folds to estimate the best alpha for that response.
    ...
    nfolds : int
        The number of cross-validation folds to run. 15 to 30 works well.
    chunklen : int
    ...
    nchunks : int
        The number of training chunks held out to test ridge parameters for each fold. The product
    ...
    corrmin : float in [0..1]
        Purely for display purposes. After each alpha is tested for each fold, the number of 
    ...
    alphas : array_like, shape (M,)
        The regularization coefficient (alpha) selected for each electrode/voxel using cross-validation.
    cv_corrs : array_like, shape (A, M, B)
        Correlation between predicted and actual responses on randomly held out portions of the training set,
        for each of A alphas, M electrodes, and B fold samples.
    valinds : array_like, shape (TH, B)
        The indices of the training data that were used as "validation" for each cv sample.
    """
    nresp, nvox = Rresp.shape
    valinds = [] # Will hold the indices into the validation data for each fold
    
    Rcmats = []
    for bi in counter(range(nfolds), countevery=1, total=nfolds):
    ...
    if not single_alpha:
        ...
        if joined is None:
            ...
            meanfoldcorrs = allRcorrs.mean(2)
            bestalphainds = np.argmax(meanfoldcorrs, 0)
            valphas = alphas[bestalphainds]
        else:
            # Find best alpha for each group of electrode
            valphas = np.zeros((nvox,))
            for jl in joined:
                # Mean across electrodes in the set, then mean across folds
        ...
        if nfolds==0:
            ...
            meanfoldcorr = allRcorrs.mean(2).mean(1)
            bestalphaind = np.argmax(meanfoldcorr)
...
def cv_ridge_shuffle(orig_STRF, Rstim, Rresp, Pstim, Presp, valpha, nfolds, chunklen,
                    corrmin=0.2, joined=None, singcutoff=1e-10, normalpha=False, single_alpha=False,
    ...
    run.  [nfolds] total regression runs will be performed.  The best alpha value for each response will be
    averaged across the cv folds to estimate the best alpha for that response.
    ...
    alphas : list or array_like, shape (A,)
        Ridge parameters that will be tested. Should probably be log-spaced. np.logspace(0, 3, 20) works well.
    nfolds : int
        The number of cv samples to run. 15 to 30 works well.
    chunklen : int
        On each sample, the training data is broken into chunks of this length. This should be a few times 
        longer than your delay/STRF. e.g. for a STRF with 3 delays, I use chunks of length 10.
    corrmin : float in [0..1]
    ...
    cv_corrs : array_like, shape (A, M, B)
        Correlation between predicted and actual responses on randomly held out portions of the training set,
        for each of A alphas, M electrodes, and B cv samples.
    valinds : array_like, shape (TH, B)
        The indices of the training data that were used as "validation" for each cv sample.
    """
    nresp, nvox = Rresp.shape
    ...
    mean_diff = np.zeros((orig_STRF.shape[0], orig_STRF.shape[1], nfolds))
    ...
    pvals = 1. - np.sum(mean_diff>0,axis=2, dtype=np.float)/nfolds
    #logger.info(pvals)
